...
import { Search, LayoutDashboard, Users, Layers, BarChart3, Network, Brain, Settings, GitCompare, User, Wand2, UserSquare2, GraduationCap, Briefcase, Trophy, X } from 'lucide-react'
...
const PAGES = [
  ...
  { id: 'page-placement', label: 'Placement', sublabel: 'Match external candidates to roles', path: '/placement', icon: <UserSquare2 className="h-4 w-4" /> },
  { id: 'page-projects', label: 'Projects', sublabel: 'Create projects and assign employees', path: '/projects', icon: <Briefcase className="h-4 w-4" /> },
  { id: 'page-learning', label: 'Learning', sublabel: 'Manage the upskilling library', path: '/learning', icon: <GraduationCap className="h-4 w-4" /> },
  { id: 'page-recognition', label: 'Recognition', sublabel: 'Give employees awards', path: '/recognition', icon: <Trophy className="h-4 w-4" /> },
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-200 dark:border-white/[0.08]">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-base text-slate-900 dark:text-white focus:outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
          />
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-white">
            <X className="h-4 w-4" />
          </button>
          <kbd className="hidden sm:flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-white/[0.08] text-[10px] font-mono text-slate-500 dark:text-slate-400">
            Esc
          </kbd>
        </div>
        ...
        <div ref={listRef} className="max-h-[420px] overflow-y-auto py-2">
          {items.length === 0 && (
            <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-10">No results for "{query}"</p>
          )}
            ...
            <div key={category}>
              ...
              {catItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={classNames('shrink-0', isActive ? 'text-signal' : 'text-slate-400 dark:text-slate-500')}>
                      {item.icon}
                    </span>
        ...
        <div className="px-4 py-2 border-t border-slate-200 dark:border-white/[0.08] flex items-center gap-3 text-[10px] text-slate-400 dark:text-slate-500">
