...
import { useUnreadMessageCount } from '../hooks/useEmployeeChat'
...
const ADMIN_NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      { to: '/audit-log', label: 'Admin Audit Log', icon: ShieldAlert, badge: 'Compliance' }
    ]
  }
]
        ...
        {groups.map((group, gIdx) => (
          <div key={gIdx} className="space-y-1">
            ...
            {group.items.map((item) => {
              const isChatLink = item.to === '/chat'
              const liveBadge = isChatLink && unreadMessages > 0 ? String(unreadMessages > 99 ? '99+' : unreadMessages) : null
              return (
                <NavLink
                  key={item.to}
                  to={item.to}
                  ...
                  {({ isActive }) => (
                    <>
                      ...
                      <span className="relative shrink-0">
                        ...
                        />
                        {liveBadge && collapsed && (
                          <span className="absolute -top-1.5 -right-1.5 h-4 min-w-[16px] px-1 rounded-full bg-rose text-white text-[9px] font-bold flex items-center justify-center shadow-sm">
                            {liveBadge}
                          </span>
                        )}
                      </span>
                      {!collapsed && (
                        <div className="flex-1 flex items-center justify-between min-w-0">
                          ...
                          {liveBadge ? (
                            <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-rose/15 text-rose min-w-[18px] text-center">
                              {liveBadge}
                            </span>
                          ) : (
                            item.badge && (
