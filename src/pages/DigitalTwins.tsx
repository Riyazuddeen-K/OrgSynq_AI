          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
              <input
                ...
                className="w-full sm:max-w-xs px-3.5 py-2.5 rounded-xl text-sm text-slate-900 dark:text-white bg-slate-50 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08] focus:border-signal/50 focus-ring placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all"
              />
              <select
                value={sort}
                onChange={(e) => setSort(e.target.value as typeof sort)}
                className="px-3.5 py-2.5 rounded-xl text-sm text-slate-900 dark:text-white bg-slate-50 dark:bg-surface-darkcard border border-slate-200 dark:border-white/[0.08] focus-ring"
              >
                ...
                <div key={t.id} className="card p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <Avatar name={t.employee?.name || '—'} src={t.employee?.photo_url} size={44} />
                      <div>
                        <p className="text-sm font-semibold text-slate-900 dark:text-white">{t.employee?.name}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{t.employee?.title}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-2 mb-4">
                    <Badge className="bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300">{t.employee?.department?.name}</Badge>
                  ...
                  <Link
                    ...
                    className="block text-center text-sm font-medium text-signal hover:underline pt-3 border-t border-slate-200 dark:border-white/[0.08]"
                  >
                    View Full Profile →
                  </Link>
                </div>
