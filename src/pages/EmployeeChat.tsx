...
export default function EmployeeChat() {
  ...
  // On mobile there isn't room for the contact list and the chat panel
  // side by side — the sidebar's fixed width was pushing the chat panel
  // (message input included) almost entirely off-screen. Below the `md`
  // breakpoint we now show one pane at a time: the list first, then the
  // chat once a contact/channel is picked, with a back button to return.
  ...
  const myIds = useMemo(
    () => new Set([currentUserId, 'my-id', 'demo-user', user?.uid ?? ''].filter(Boolean)),
    [currentUserId, user?.uid]
  )
  const employeeIds = useMemo(() => new Set(employees.map((e) => e.id)), [employees])

  // People who've messaged me but have no `employees` record of their own —
  // an admin account, for instance, since admins aren't stored as
  // employees. Without this, an admin's DM had nowhere to appear: the
  // sidebar only ever listed real employees, so the thread was completely
  // unreachable even though the message itself was saved correctly.
  // sender_name/sender_photo are captured on every message specifically so
  // a contact like this can still be identified and replied to.
  const externalContacts = useMemo(() => {
    const byId = new Map<string, { id: string; name: string; photo_url?: string; lastAt: number }>()
    for (const m of messages) {
      if (m.sender_id && !myIds.has(m.sender_id) && !employeeIds.has(m.sender_id)) {
        const at = new Date(m.created_at).getTime()
        const existing = byId.get(m.sender_id)
        if (!existing || at > existing.lastAt) {
          byId.set(m.sender_id, { id: m.sender_id, name: m.sender_name || 'Team Member', photo_url: m.sender_photo, lastAt: at })
        }
      }
    }
    return Array.from(byId.values())
  }, [messages, myIds, employeeIds])

  // Unified, unfiltered contact list (real employees + external senders
  // like admins) — the source of truth for both the sidebar and the chat
  // header, so an active conversation never disappears just because it's
  // filtered out of the visible/searched list below.
  const allContacts = useMemo(() => {
    const real = employees
      .filter((e) => !(profile?.employee_id && e.id === profile.employee_id))
      .map((e) => ({
        id: e.id,
        name: e.name,
        title: e.title,
        photo_url: e.photo_url,
        departmentLabel: e.department?.name?.slice(0, 4) || 'Org',
        isEmployee: true as const
      }))
    const external = externalContacts.map((c) => ({
      id: c.id,
      name: c.name,
      title: 'Admin',
      photo_url: c.photo_url,
      departmentLabel: 'Admin',
      isEmployee: false as const
    }))

    // Whoever most recently exchanged a message with me comes first —
    // WhatsApp-style — so a new message doesn't get buried in
    // alphabetical/directory order.
    const lastActivity = new Map<string, number>()
    for (const m of messages) {
      let otherId: string | null = null
      if (myIds.has(m.recipient_id) && m.sender_id && !myIds.has(m.sender_id)) otherId = m.sender_id
      else if (m.sender_id && myIds.has(m.sender_id) && m.recipient_id !== 'general-channel') otherId = m.recipient_id
      if (!otherId) continue
      const at = new Date(m.created_at).getTime()
      if (!lastActivity.has(otherId) || at > (lastActivity.get(otherId) as number)) lastActivity.set(otherId, at)
    }

    return [...real, ...external].sort((a, b) => {
      const aTime = lastActivity.get(a.id) ?? -1
      const bTime = lastActivity.get(b.id) ?? -1
      if (aTime !== bTime) return bTime - aTime
      return a.name.localeCompare(b.name)
    })
  }, [employees, externalContacts, profile?.employee_id, messages, myIds])

  const activeContactDisplay = useMemo(
    () => allContacts.find((c) => c.id === activeRecipientId) || null,
    [allContacts, activeRecipientId]
  )
  ...
  const filteredColleagues = useMemo(() => {
    ...
    return allContacts.filter(
      (c) => c.name.toLowerCase().includes(q) || c.title.toLowerCase().includes(q) || c.departmentLabel.toLowerCase().includes(q)
    )
  ...
  function handleClearChat() {
    const targetName = activeRecipientId === 'general-channel' ? '# general-channel' : activeContactDisplay?.name || 'this contact'
        ...
        <div className="card h-full flex overflow-hidden border border-slate-200/80 dark:border-white/[0.08] shadow-card">
          ...
          <div
            className={classNames(
              'w-full md:w-80 border-r border-slate-200 dark:border-white/[0.08] flex-col shrink-0 bg-slate-50/50 dark:bg-black/15',
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin">
              {/* General Channel */}
              <button
                onClick={() => selectRecipient('general-channel')}
                className={classNames(
              ...
              <div className="pt-3 pb-1 px-3 flex items-center justify-between">
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  Contacts ({filteredColleagues.length})
                </p>
              </div>
              ...
              {filteredColleagues.map((emp) => {
                ...
                return (
                  <button
                    key={emp.id}
                    onClick={() => selectRecipient(emp.id)}
                    className={classNames(
          ...
          <div
            className={classNames(
              'flex-1 min-h-0 flex-col bg-surface-lightcard dark:bg-surface-darkcard overflow-hidden',
          ...
          >
            ...
            <div className="px-5 py-3 border-b border-slate-200 dark:border-white/[0.08] flex items-center justify-between bg-slate-50/30 dark:bg-white/[0.01]">
              <div className="flex items-center gap-3 min-w-0">
                <button
                  ...
                  className="md:hidden -ml-1.5 mr-0.5 h-8 w-8 shrink-0 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-white/[0.06] focus-ring"
                >
                ...
                {activeRecipientId === 'general-channel' ? (
                  ...
                  <div className="relative">
                    <Avatar name={activeContactDisplay?.name || 'User'} src={activeContactDisplay?.photo_url} size={40} />
                    <span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full bg-teal ring-2 ring-white dark:ring-surface-darkcard" />
                  </div>
                )}
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-display font-semibold text-sm text-slate-900 dark:text-white">
                      {activeRecipientId === 'general-channel' ? '# general-channel' : activeContactDisplay?.name}
                    </h3>
                  </div>
                  <p className="text-[11px] text-slate-400">
                    {activeRecipientId === 'general-channel'
                      ? 'Open to all team members · Company-wide'
                      : `${activeContactDisplay?.title || 'Employee'} · ${activeContactDisplay?.departmentLabel || 'Department'}`}
                  </p>
                </div>
              </div>
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1">
                    {activeContactDisplay
                      ? `Send a message to ${activeContactDisplay.name} to connect directly.`
                  ...
                  >
                    {!isMe && (
                      <Avatar
                        ...
                        src={msg.sender_photo || (activeContactDisplay?.id === msg.sender_id ? activeContactDisplay.photo_url : undefined)}
                        size={28}
            ...
            <form onSubmit={handleSend} className="p-4 border-t border-slate-200 dark:border-white/[0.08] flex items-center gap-2">
              <input
                ...
                placeholder={`Message ${
                  activeRecipientId === 'general-channel' ? '#general-channel' : activeContactDisplay?.name || 'colleague'
                } (Press Enter to send)...`}
