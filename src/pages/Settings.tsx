...
import {
  Sun, Moon, Database, CheckCircle2, AlertTriangle, User, Lock, Mail, Eye, EyeOff,
  Shield, Trash2, Pencil, Plus, Building2, Key, Cpu, Sparkles, RefreshCw
} from 'lucide-react'
...
export default function Settings() {
  ...
  const [customKeys, setCustomKeys] = useState<string[]>(getCustomGeminiKeys())
  ...
  return (
    <div>
      ...
      <div className="p-4 md:p-8 max-w-2xl space-y-5">
        ...
        {/* Gemini AI Multi-Key Pool & Failover Manager */}
        <div className="card p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-display font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                <Cpu className="h-4 w-4 text-signal" /> Gemini AI Failover Pool & Resilience
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Automatically rotates between multiple API keys and fallback models if Google's server is overloaded (429 / 503 / 500).
              </p>
            </div>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-teal/15 text-teal">
              <Sparkles className="h-3 w-3" /> Auto-Failover Active
            </span>
          </div>

          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200/60 dark:border-white/[0.06] flex items-center justify-between text-xs">
            <div>
              <p className="font-semibold text-slate-800 dark:text-slate-200">Active Pool Slot: {keyInfo.index} of {keyInfo.total}</p>
              <p className="text-[11px] text-slate-400 font-mono mt-0.5">Current Key: {keyInfo.masked}</p>
            </div>
            <button
              onClick={() => setKeyInfo(getActiveKeyInfo())}
              className="p-1.5 rounded-lg text-slate-500 hover:text-signal hover:bg-slate-200 dark:hover:bg-white/10 transition-colors"
          ...
          <div className="space-y-2">
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
              Add Backup Gemini API Key
            </label>
            <div className="flex items-center gap-2">
              <input
                placeholder="AIzaSy... or custom Gemini API Key"
              ...
              <button
                ...
                onClick={() => {
                  ...
                  saveCustomGeminiKey(customKeyInput)
                  setCustomKeyInput('')
                  setCustomKeys(getCustomGeminiKeys())
                  setKeyInfo(getActiveKeyInfo())
                }}
                className="px-4 py-2 rounded-xl bg-signal text-white text-xs font-semibold hover:bg-signal/90 shadow-sm shrink-0"
              >
                Add to Pool
              </button>
            </div>
          ...
          {customKeys.length > 0 && (
            <div className="space-y-1.5 pt-1">
              <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Custom Pool Keys:</p>
              <div className="space-y-1">
                {customKeys.map((k, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-100 dark:bg-white/[0.05] text-xs font-mono">
                    <span>{k.slice(0, 6)}...{k.slice(-4)}</span>
                    <button
                      onClick={() => {
                        removeCustomGeminiKey(k)
                        setCustomKeys(getCustomGeminiKeys())
                        setKeyInfo(getActiveKeyInfo())
                      }}
