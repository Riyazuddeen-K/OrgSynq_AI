...
import { ArrowLeft, Mail, MapPin, Briefcase, Award, MessageCircle } from 'lucide-react'
import Topbar from '../components/Topbar'
import Avatar from '../components/Avatar'
import { ProgressBar, Badge, LoadingState } from '../components/Primitives'
import AttritionRiskPanel from '../components/AttritionRiskPanel'
import OneOnOnePrepModal from '../components/OneOnOnePrepModal'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="card p-6 lg:col-span-1 h-fit">
              <div className="flex flex-col items-center text-center">
                <Avatar name={employee.name} src={employee.photo_url} size={72} />
              ...
              <button
                ...
                className="w-full mt-5 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-signal/10 text-signal text-sm font-medium hover:bg-signal/20 transition-colors focus-ring"
              >
                <MessageCircle className="h-4 w-4" /> Prep 1:1
              </button>
            </div>
            ...
            <div className="lg:col-span-3">
              <AttritionRiskPanel employee={employee} twin={twin} />
            </div>
          </div>
        )}
      </div>

      {showPrepModal && employee && (
        <OneOnOnePrepModal employee={employee} twin={twin} onClose={() => setShowPrepModal(false)} />
      )}
    </div>
  )
}
