...
import { Search, Download, LayoutGrid, List as ListIcon, UserPlus, Trash2, Pencil } from 'lucide-react'
...
export default function Employees() {
  const { employees, loading, error, addEmployee, deleteEmployee, updateEmployee } = useEmployees()
  ...
  function exportCsv() {
    const header = ['Name', 'Title', 'Department', 'Location', 'Status', 'Performance', 'Burnout', 'Attrition Risk', 'Experience (yrs)', 'Skills']
    ...
    <div>
        ...
        <div className="flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 rounded-xl text-sm text-slate-900 dark:text-white bg-slate-50 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08] focus:border-signal/50 focus-ring placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all"
            />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="px-3 py-2.5 rounded-xl text-sm text-slate-900 dark:text-white bg-slate-50 dark:bg-surface-darkcard border border-slate-200 dark:border-white/[0.08] focus-ring"
            >
              <option>All</option>
              {departments.map((d) => (
            ...
            <div className="flex bg-slate-100 dark:bg-white/[0.06] rounded-xl p-1 border border-slate-200/60 dark:border-white/[0.06]">
              <button
                onClick={() => setView('grid')}
                className={`h-8 w-8 rounded-lg flex items-center justify-center focus-ring ${view === 'grid' ? 'bg-signal text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white'}`}
              >
                <LayoutGrid className="h-4 w-4" />
              </button>
              <button
                onClick={() => setView('list')}
                className={`h-8 w-8 rounded-lg flex items-center justify-center focus-ring ${view === 'list' ? 'bg-signal text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white'}`}
              >
            ...
            {filtered.map((e) => (
              <Link key={e.id} to={`/employees/${e.id}`} className="card p-4 hover:border-signal/40 transition-colors group">
                <div className="flex items-start justify-between mb-3">
                  <Avatar name={e.name} src={e.photo_url} size={44} />
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {(e.skills ?? []).length > 3 && (
                      <span className="text-[10px] text-black/35 dark:text-white/30 self-center">
                        +{(e.skills ?? []).length - 3}
                      </span>
                    )}
                    {(e.skills ?? []).length === 0 && (
                      <span className="text-[10px] text-black/30 dark:text-white/25">No skills listed</span>
                    )}
                  </div>
                  <span className="text-[10px] text-black/40 dark:text-white/30 shrink-0 ml-2">
                    {e.experience_years ?? 0}y exp
                  </span>
                </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-black/40 dark:text-white/40 border-b border-surface-lightborder dark:border-surface-darkborder">
                  ...
                  <th className="px-4 py-3 font-medium text-right">Exp</th>
                  ...
                  >
                    <td className="px-4 py-3">
                      <Link to={`/employees/${e.id}`} className="flex items-center gap-3">
                        <Avatar name={e.name} src={e.photo_url} size={32} />
                        <div>
                    ...
                    <td className="px-4 py-3 max-w-[200px]">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {(e.skills ?? []).length === 0 && <span className="text-[10px] text-black/30 dark:text-white/25">—</span>}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-right">{e.experience_years ?? 0}y</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="h-7 w-7 rounded-md inline-flex items-center justify-center text-black/30 dark:text-white/30 hover:text-signal hover:bg-signal/10 focus-ring"
                          title="Edit employee"
                        >
                          <Pencil className="h-3.5 w-3.5" />
                        </button>
      ...
      {editingEmployee && (
        <AddEmployeeModal
          ...
          onSubmit={(input) => updateEmployee(editingEmployee.id, input)}
        />
      )}
    </div>
  )
