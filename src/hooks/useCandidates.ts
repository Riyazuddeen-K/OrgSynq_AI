...
export function useCandidates() {
  ...
  const addCandidate = useCallback(async (input: NewCandidateInput) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'candidates'), { ...rest, ...optionalField('notes', notes), created_at: Timestamp.now() })
      await logActivity({
  ...
  const updateCandidate = useCallback(async (id: string, input: NewCandidateInput) => {
    ...
    try {
      const { notes, ...rest } = input
      // Notes cleared back to blank actually removes the field here
      // (deleteField()), rather than silently keeping the old value.
      await updateDoc(doc(db, 'candidates', id), { ...rest, notes: valueOrDeleteField(notes) })
      await logActivity({
        action: 'candidate_added',
