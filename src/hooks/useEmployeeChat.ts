...
// Shared subscription to the full `employee_messages` collection — both
// useEmployeeChat (the full chat page) and useUnreadMessageCount (the
// sidebar badge) need the same raw data, so this keeps the Firestore/
// localStorage sync logic in one place instead of duplicated.
function useRawEmployeeMessages() {
        ...
        (snap) => {
          const remote = snap.docs.map((d) => {
            ...
            return {
              ...
              // Preserve the tri-state as stored: true/false track real
              // read status, undefined means "not tracked" (older
              // messages written before read-tracking existed, or the
              // general channel) — don't collapse it to false, or every
              // pre-existing message would suddenly count as unread.
...
// Lightweight — just a live unread count for the "Team Chat" nav badge, so
// it's visible from the app sidebar without opening the chat page. Shares
// the same subscription/read-tracking rules as the full chat hook below.
export function useUnreadMessageCount(): number {
  const { profile, user } = useAuth()
  const { messages } = useRawEmployeeMessages()
  ...
  return useMemo(() => {
    ...
    for (const m of messages) {
      if (m.read === false && myIds.has(m.recipient_id) && m.sender_id && !myIds.has(m.sender_id)) count++
    }
    return count
...
export function useEmployeeChat(activeRecipientId: string, _activeColleague: Employee | null = null) {
  const { profile, user } = useAuth()
  const { messages, setMessages, loading } = useRawEmployeeMessages()
  ...
  const sendMessage = useCallback(
    ...
    [currentUserId, currentUserName, currentUserPhoto, setMessages]
  )
  ...
  const clearChat = useCallback(
    ...
    [currentUserId, user?.uid, messages, setMessages]
  )
  ...
  const markThreadRead = useCallback(
    ...
    [messages, currentUserId, user?.uid, setMessages]
  )
