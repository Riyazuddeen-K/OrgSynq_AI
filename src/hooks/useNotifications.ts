...
export function useNotifications() {
  ...
  useEffect(() => {
    ...
    //
    // The one exception: notifications addressed to the admin's own
    // identity (e.g. a chat DM — admin accounts aren't `employees` records,
    // so a message to one is targeted by their raw Firebase UID, same as
    // any other targeted notification) still need to reach their own bell.
    if (role !== 'employee') {
      const myIds = new Set([user?.uid, profile?.employee_id].filter(Boolean))
      const q = query(collection(db, 'notifications'), orderBy('created_at', 'desc'), limit(50))
      const unsub = onSnapshot(q, (snap) => {
        const relevant = fromSnap(snap).filter(
          (n) =>
            !n.audience_employee_id ||
            n.audience_employee_id === 'admin' ||
            n.audience_employee_id === 'all' ||
            myIds.has(n.audience_employee_id)
        )
    ...
    // people). Two plain-equality listeners (not `in`) merged
    // client-side, so there's only ever one filter value per query —
    // but each still combines that filter with orderBy('created_at') on
    // a different field, which Firestore can only serve with a composite
    // index. That index is defined in firebase/firestore.indexes.json
    // under the `notifications` collection; if it isn't deployed to your
    // Firebase project (Vercel does not do this for you — see the
    // README's Firestore index note), these two queries fail silently
    // and no notification, of any kind, ever reaches an employee's bell.
    ...
    const unsubBroadcast = onSnapshot(
      ...
      (err) => {
        console.warn(
          '[OrgSynq Notifications] Broadcast listener failed — likely a missing Firestore composite index (see firebase/firestore.indexes.json):',
          err.message
        )
        broadcastLoaded = true
        merge()
    ...
    if (employeeId) {
      unsubPersonal = onSnapshot(
        ...
        (err) => {
          console.warn(
            '[OrgSynq Notifications] Personal listener failed — likely a missing Firestore composite index (see firebase/firestore.indexes.json):',
            err.message
          )
          personalLoaded = true
          merge()
  ...
  }, [role, profile?.employee_id, user?.uid])
