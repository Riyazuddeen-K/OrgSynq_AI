...
- **1:1 meeting assistant** — generates AI talking points for a manager's next 1:1 with any employee, pulling from their digital twin trends, recent pulse survey answers, and open insights about them; logged 1:1s feed the Manager Attention panel on Command Center
...
This populates Firestore with 8 departments, 50 employees (with a realistic manager hierarchy, sample skills, and years of experience), digital twins, sample AI insights, sample simulations, a 7-month health trend, notifications, and 6 sample external candidates for the Placement pool. It's safe to re-run — it clears each collection first (except `users`, which it never touches). Check it worked: Firebase Console → Firestore Database → Data tab → you should see an `employees` collection with 50 documents.
