...
5. You'll see a `firebaseConfig` object — keep this tab open, you'll need these values in step 4 below.
...
## 3. Create the notifications index

The notification bell (kudos, awards, project assignments, chat messages, etc.) queries Firestore by `audience_employee_id` sorted by `created_at`, which Firestore can only serve with a composite index — without it, employees' bells silently stay empty. This is a one-time setup step per Firebase project; Vercel deploys don't touch Firestore config, so this won't get created just by deploying.

1. In Firestore Database, click the **Indexes** tab → **Composite** → **Create index**.
2. Collection ID: `notifications`. Add two fields: `audience_employee_id` (Ascending), then `created_at` (Descending). Query scope: **Collection**. Click **Create**.
3. It takes a minute or two to finish building — the definition is also saved in `firebase/firestore.indexes.json` for reference, or if you use the Firebase CLI: `firebase deploy --only firestore:indexes`.

(Easiest alternative: sign in as an employee, open the browser console, and trigger a personal notification — Firestore's error includes a direct "create this index" link that pre-fills everything for you.)
...
(The `VITE_` versions are read by the browser app. The plain versions are read by the local seed script in step 5 — same values, just duplicated because Vite only exposes `VITE_`-prefixed vars to the browser bundle.)

`.env` is already listed in `.gitignore`, so your keys never get pushed to GitHub.

## 5. Create your first account, then seed sample data
...
1. Start the app first (`npm run dev`, see step 6) and sign up for an account on the login screen — any email/password (6+ characters) works. Firebase doesn't verify the email address.
...
## 8. Deploy on Vercel (recommended)
...
3. Before clicking Deploy, expand **Environment Variables** and add the six `VITE_FIREBASE_*` variables, plus `VITE_GEMINI_API_KEY` if you're using the chat assistant (the plain `FIREBASE_*` ones are only needed locally for `npm run seed`, not on Vercel). Have more than one Gemini key for failover? Use `VITE_GEMINI_API_KEYS` instead (comma-separated, e.g. `key-one,key-two,key-three`) — see the Gemini failover note below.
...
- **Ask OrgSynq (or Prediction) shows a Gemini error / model not found**: this app uses `@google/genai` (Google's current SDK) and automatically cascades through several model aliases (`gemini-2.0-flash`, `gemini-1.5-flash`, `gemini-1.5-flash-8b`, `gemini-flash-latest`) if one isn't available — the list lives in `MODELS_CASCADE` at the top of `src/lib/geminiClient.ts` if you ever need to update it. Also double-check the key went into `.env`, not `.env.example` (see the Gemini key note in step 4 above), and that you restarted `npm run dev` after adding it.
- **Using multiple Gemini API keys for failover**: set `VITE_GEMINI_API_KEYS` (plural) to a comma-separated list instead of the single `VITE_GEMINI_API_KEY` — e.g. `VITE_GEMINI_API_KEYS=key-one,key-two,key-three` (both locally in `.env` and in Vercel's Environment Variables). Every AI feature (Ask OrgSynq, Prediction, Placement, career paths, 1:1 prep, insights) automatically rotates to the next key in the pool the moment one hits a quota/overload error (429/503) or is rejected as invalid (401) — no code change or redeploy needed, and it only falls back to the local deterministic response generator if every key in the pool fails. You can also add backup keys per-browser from **Settings → Gemini AI Failover Pool & Resilience** without touching env vars at all, and see exactly which key slot is currently active there.
...
- **`npm run seed` fails with `permission-denied`**: it needs `SEED_EMAIL` / `SEED_PASSWORD` in `.env` for an account that already exists (see step 5) — the rules block anonymous writes.
- **Employees never see notifications in the bell (kudos, awards, project assignments, chat messages, etc.), even right after they're triggered**: this almost always means the composite index from step 3 hasn't been created in your Firebase project yet — deploying to Vercel does not create it for you. Open the browser console while signed in as an employee; a failed query there logs `[OrgSynq Notifications] ... listener failed` with the Firestore error, and Firestore's `failed-precondition` error includes a direct "create this index" link. Admin accounts are unaffected by this since their notification query doesn't need the index.
