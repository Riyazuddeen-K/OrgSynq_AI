...
- **Ask OrgSynq (or Prediction) shows a Gemini error / model not found**: this app uses `@google/genai` (Google's current SDK) and automatically cascades through several model aliases (`gemini-2.0-flash`, `gemini-1.5-flash`, `gemini-1.5-flash-8b`, `gemini-flash-latest`) if one isn't available — the list lives in `MODELS_CASCADE` at the top of `src/lib/geminiClient.ts` if you ever need to update it. Also double-check the key went into `.env`, not `.env.example` (see the Gemini key note in step 3 above), and that you restarted `npm run dev` after adding it.
