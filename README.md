...
3. Before clicking Deploy, expand **Environment Variables** and add the six `VITE_FIREBASE_*` variables, plus `VITE_GEMINI_API_KEY` if you're using the chat assistant (the plain `FIREBASE_*` ones are only needed locally for `npm run seed`, not on Vercel). If you're using multiple Gemini keys for failover (see **Multiple API keys / automatic failover** below), add `VITE_GEMINI_API_KEY_2`, `_3`, etc. here too — Vercel env vars work exactly like a local `.env` file, since Vite bakes them in at build time either way. Changing env vars later requires a redeploy to take effect.
...
Every AI call (Ask OrgSynq, Prediction, Placement, career paths, the 1:1 assistant) automatically tries each configured key in order — immediately, with no delay — before falling back to a short wait-and-retry pass across all of them. This helps most when the keys come from **separate Google accounts or Cloud projects**, since each has its own quota; two keys under the same account/project share the same quota and won't help with a 429, though they can still help route around the `AQ.` key auth issue if only one of your keys is affected by it. This is entirely optional — a single `VITE_GEMINI_API_KEY` still works exactly as before.
